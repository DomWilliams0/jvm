...
mod classpath_zip {
    ...
    static ARCHIVE: Mutex<Option<ZipArchive<Cursor<&'static [u8]>>>> =
    ...
    pub fn read_file(path: &Path) -> Option<Result<Vec<u8>, std::io::Error>> {
        ...
        if guard.is_none() {
            ...
            *guard = Some(
                ZipArchive::new(Cursor::new(CLASSPATH_ZIP)).expect("failed to read classpath zip"),
            );
        }

        let archive = guard.as_mut().unwrap(); // definitely initialized now

        let ret = if let Ok(mut file) = archive.by_name(&*path.to_string_lossy()) {
